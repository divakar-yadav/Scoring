...
import './StudentsWithDisabilitiesChart.css'
HighchartsMore(Highcharts);
SolidGauge(Highcharts);

const StudentsWithDisabilitiesChart = ({ row }) => {
  // Extract the relevant value from the row and convert to percentage
  const disabilitiesPercent = parseFloat((row["Percent Students with Disabilities"] * 100).toFixed(2));
...
export default StudentsWithDisabilitiesChart;
